...
public class ShowPrivacyDialogView extends AppCompatDialog implements View.OnClickListener {
    ...
    private void initViews() {
        ...
        spannableString.setSpan(new ForegroundColorSpan(Color.parseColor("#1677FF")),lastStr.length(),lastStr.length()+privacyStr.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        spannableString.setSpan(new ForegroundColorSpan(Color.parseColor("#1677FF")),lastStr.length()+privacyStr.length()+andStr.length(),lastStr.length()+privacyStr.length()+andStr.length()+agreeStr.length(),Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
